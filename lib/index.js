...
const { getJson } = require('../fetch/load');
const upgrade = NativeModules.XSYUpgradeManager;


 async function copyFolder(currentVersion,versionPath,folderName){
...
export default async function checkUpgrade(checkUpdateUrl) {
    console.log('Network detecting...');
    ...
    let currentVersion = await upgrade.currentVersion(),
        baseVersion = await upgrade.baseVersion(),
        url = `${checkUpdateUrl}?currentVersion=${currentVersion}&type=${Platform.OS === 'ios' ? 2 : 1}&_=${+new Date}`;
    console.log(url);
